...
#define OUTPUT_BUF_SIZE  4096    /* choose an efficiently fwrite'able size */
	...
	*/
	...
	static unsigned char* readJPGStream(std::istream& fin, int& width_ret, int& height_ret, int& numComponents_ret)
	{
		unsigned char *imageData = NULL;

		imageData = JPEGREADER::simage_jpeg_load(fin, &width_ret, &height_ret, &numComponents_ret);

		if (imageData == NULL) return 0L;
...
JPGReader::JPGReader() : ImageReader()
{
}

Image *JPGReader::readImage(const std::string &filename)
{
	int width;
	int height;
	int components;
	...
	const std::string path = vr::FileSystem::convertToNativeFilePath(filename);

	std::ifstream istream;
	istream.open(path.c_str(), std::ios_base::binary);
	if (!istream.is_open())
	{
		std::cerr << "Unable to open JPEG file: " << filename << std::endl;
		return 0L;
	}

	data = JPEGREADER::readJPGStream(istream, width, height, components);


	if (data == 0)
	{
		std::cerr << "Error while parsing JPEG file: " << filename << std::endl;
		return 0L;
	}
